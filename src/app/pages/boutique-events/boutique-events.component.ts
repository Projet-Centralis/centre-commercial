...
export class BoutiqueEventsComponent implements OnInit {
  ...
  loadEvents(): void {
    ...
    // Utiliser la route avec participants pour avoir les statistiques
    this.eventService.getBoutiqueEventsWithParticipants().subscribe({
  ...
  applyFilters(): void {
    ...
    switch(this.sortBy) {
      ...
      case 'statut':
        ...
        break;
      case 'participants':
        filtered.sort((a, b) => (b.nombre_participants || 0) - (a.nombre_participants || 0));
        break;
    }
  ...
  openDetailsModal(event: any): void {
    this.currentEvent = event;
    this.loading = true;
    
    this.eventService.getEventDetails(event._id).subscribe({
      next: (response: ApiResponse<any>) => {
        if (response.success && response.data) {
          this.selectedEventDetails = response.data;
          this.showDetailsModal = true;
        } else {
          this.errorMessage = response.message || 'Erreur lors du chargement des détails';
        }
        this.loading = false;
      },
      error: (error) => {
  ...
  formatDateForInput(dateString: string): string {
    return this.eventService.formatDateForInput(dateString);
  }
  ...
  getStatusColor(statut: string): string {
    return this.eventService.getStatusColor(statut);
  }

  getStatusText(statut: string): string {
    return this.eventService.getStatusText(statut);
  }

  getProgressPercentage(event: any): number {
    return this.eventService.getProgressPercentage(event.nombre_participants || 0, event.capacite_max);
  }

  getProgressColor(event: any): string {
    const percentage = this.getProgressPercentage(event);
    return this.eventService.getProgressColor(percentage);
  }
  ...
  // Méthodes pour les statistiques
  getPendingEventsCount(): number {
    return this.events.filter(e => e.statut === 'en_attente').length;
  }
  ...
  getTotalParticipants(): number {
    return this.events.reduce((sum, e) => sum + (e.nombre_participants || 0), 0);
  }

  // Méthodes pour les participants
  getParticipantsList(): any[] {
    return this.selectedEventDetails?.participants || [];
  }

  getTauxRemplissage(): number {
    return this.selectedEventDetails?.statistiques?.taux_remplissage || 0;
  }

  getPlacesRestantes(): number {
    return this.selectedEventDetails?.statistiques?.places_restantes || 0;
  }
}