...
import { RouterModule } from '@angular/router';
import { LoyerService, ContratLoyer, PaiementLoyer } from '../../services/loyer.service';
import { BoutiqueService } from '../../services/boutique.service'; // À importer
  ...
  template: `
    <div class="loyer-container">
      ...
      <div *ngIf="!isLoading" class="loyer-content">
        ...
        <!-- Actions header avec bouton Nouveau contrat -->
        <div class="actions-header">
          <h3>
            <i class="bi bi-list-ul"></i>
            Contrats par boutique
          </h3>
          <button class="create-btn" (click)="openCreateContratModal()">
            <i class="bi bi-plus-circle"></i>
            Nouveau contrat
          </button>
        </div>
    ...
    <div *ngIf="showCreateContratModal" class="modal-overlay" (click)="closeCreateContratModal()">
      <div class="modal-content" (click)="$event.stopPropagation()">
        <div class="modal-header">
          ...
          <button class="modal-close" (click)="closeCreateContratModal()">
            <i class="bi bi-x"></i>
          </button>
        </div>

        <div class="modal-body">
          <form (ngSubmit)="submitContrat()" class="modal-form">
            <!-- Sélection de la boutique -->
            <div class="form-group">
              <label>Boutique *</label>
              <select [(ngModel)]="contratForm.boutique" name="boutique" required class="filter-select" (change)="onBoutiqueChange()">
            ...
            <!-- Alerte si contrat déjà actif -->
            <div *ngIf="selectedBoutiqueHasActiveContract" class="warning-note">
              <i class="bi bi-exclamation-triangle-fill"></i>
              <span>Attention : Cette boutique a déjà un contrat actif. La création d'un nouveau contrat désactivera automatiquement l'ancien.</span>
            </div>

            <div class="form-row">
              <div class="form-group">
                <label>Montant mensuel (€) *</label>
                <input type="number" [(ngModel)]="contratForm.montant_mensuel" name="montant_mensuel" min="0" step="0.01" required>
              </div>

              <div class="form-group">
                <label>Jour d'échéance *</label>
                <input type="number" [(ngModel)]="contratForm.jour_echeance" name="jour_echeance" min="1" max="31" required>
              </div>
            </div>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="date" [(ngModel)]="contratForm.date_debut" name="date_debut" required>
              </div>

              <div class="form-group">
                <label>Date de fin *</label>
                <input type="date" [(ngModel)]="contratForm.date_fin" name="date_fin" required>
              </div>
            </div>

            <div class="form-group checkbox-group">
              <label>
                <input type="checkbox" [(ngModel)]="contratForm.is_active" name="is_active">
                Contrat actif (sera le contrat en cours)
              </label>
            </div>
            ...
            <div class="modal-actions">
              <button type="button" class="cancel-btn" (click)="closeCreateContratModal()">Annuler</button>
              <button type="submit" class="submit-btn" [disabled]="isLoading">
                <span *ngIf="!isLoading">
                  <i class="bi bi-check-circle"></i> Créer le contrat
                </span>
                <span *ngIf="isLoading">
                  <i class="bi bi-arrow-clockwise spin"></i> Création...
                </span>
              </button>
            </div>
          </form>
...
export class AdminLoyersComponent implements OnInit {
  contrats: any[] = [];
  boutiques: any[] = []; // Liste des boutiques pour le sélecteur
  isLoading = false;
  errorMessage = '';
  successMessage = '';
  ...
  showCreateModal = false;
  showCreateContratModal = false; // Nouvelle modal
  selectedBoutiqueHasActiveContract = false; // Flag pour alerte
  ...
  constructor(
    ...
    private boutiqueService: BoutiqueService // À injecter
  ) {}

  ngOnInit(): void {
    this.loadData();
    this.loadBoutiques(); // Charger les boutiques pour le sélecteur
  }
  ...
  loadBoutiques(): void {
    this.boutiqueService.getAllBoutiques().subscribe({
      next: (response: any) => {
        // Adapter selon le format de réponse de votre service
  ...
  openCreateContratModal(): void {
    ...
    this.selectedBoutiqueHasActiveContract = false;
  ...
  onBoutiqueChange(): void {
    // Vérifier si la boutique sélectionnée a déjà un contrat actif
    const selectedBoutiqueId = this.contratForm.boutique;
    const existingActiveContract = this.contrats.find(
      c => c.boutique?._id === selectedBoutiqueId && c.is_active
    );
    this.selectedBoutiqueHasActiveContract = !!existingActiveContract;
  }
  ...
  submitContrat(): void {
    ...
    const contratData = {
      ...this.contratForm,
      date_debut: new Date(this.contratForm.date_debut),
      date_fin: new Date(this.contratForm.date_fin)
    };

    this.loyerService.createContrat(contratData).subscribe({
      next: (response) => {
        if (response.success) {
