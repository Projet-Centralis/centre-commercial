...
import { Component, OnInit } from '@angular/core';
import { CommonModule } from '@angular/common';
import { EventService, Event, ApiResponse } from '../../services/event.service'; // Importez Event depuis le service
...
export class EventsAdminComponent implements OnInit {
  ...
  buildCalendar(): void {
    ...
    const startDate = new Date(firstDay);
    startDate.setDate(1 - (startOffset === 0 ? 6 : startOffset - 1));
  ...
  rejectEvent(eventId: string): void {
    this.eventsService.rejectEvent(eventId).subscribe({
      next: (res: ApiResponse<Event>) => {
        if (res.success) {
          this.showMessage('success', 'Événement refusé');
          this.loadAllEvents();
